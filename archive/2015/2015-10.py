...
def look_and_say(stringa: str) -> str:
    # use the regex /(\d)\1*/g and iterate the matches
    matches = re.finditer(r"(\d)\1*", stringa)

    newstring = ""

    for match in matches:
        match = match.group()
        newstring += str(len(match)) + match[0]
